...
class IEMOCAPDataset(Dataset):
    def __init__(self, path, train=True, is_Sn=False, missing_rate=0.):
    ...
    def __getitem__(self, index):
        vid = self.keys[index]
        seq_len = len(self.videoLabels[vid])
        Sn = [torch.ones(seq_len, 1)] * 3
        # print(seq_len)

        if self.is_Sn:
            Sn = get_sn(3, seq_len, self.missing_rate)

        # print(Sn)
        # print(torch.tensor(self.videoText[vid], dtype=torch.float32).shape)
